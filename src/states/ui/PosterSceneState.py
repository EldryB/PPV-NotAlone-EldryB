...
class PosterSceneState(BaseState):
    def __init__(self, ui_stack, on_complete=None):
        super().__init__(None)
        self.ui_stack = ui_stack
        self.on_complete = on_complete
        
        self.bg_img = settings.TEXTURES.get("background_anphitheater")
        if self.bg_img:
            self.bg_img = pygame.transform.scale(self.bg_img, (settings.VIRTUAL_WIDTH, settings.VIRTUAL_HEIGHT))
            
        self.poster_img = settings.TEXTURES.get("cartel_pista")
        if self.poster_img:
            self.poster_img = pygame.transform.scale(self.poster_img, (512, 272))
    ...
    def start_dialogue(self):
        ...
        self.ui_stack.push(DialogueState(
            self.ui_stack,
            lines=["El trabajo mas importante de un Medico es darle mas TIEMPO a sus pacientes"],
            speaker="Cartel",
    ...
    def render(self, surface):
        ...
        if self.poster_img:
            px = (settings.VIRTUAL_WIDTH - 512) // 2
            py = (settings.VIRTUAL_HEIGHT - 272) // 2
            surface.blit(self.poster_img, (px, py))
